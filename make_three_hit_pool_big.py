...
def make_three_hit_factor_pool_big(stocks0, dates):
    ...
    for trade_date_num_i in range(len(trade_date_list)):
        ...
        ##########################################
        # lowprice
        f_part_line = standard_line(OperatingProfitGrowRate_df.values[i]) + standard_line(
            NPParentCompanyGrowRate_df.values[i])

        f_part_line = numpy.where(judge_line > 0, f_part_line, numpy.nan)
        gro_line = arank(f_part_line, reverse=-1) / len(
            f_part_line[numpy.isfinite(f_part_line)])
        f_part_line = ETOP_df.values[i]
        f_part_line = numpy.where(judge_line > 0, f_part_line, numpy.nan)
        bt_line = arank(f_part_line, reverse=-1) / len(
            f_part_line[numpy.isfinite(f_part_line)])
        f_part_line = PEreturn_factor_df.values[i]
        f_part_line = numpy.where(judge_line > 0, f_part_line, numpy.nan)
        pereturn_line = arank(f_part_line, reverse=-1) / len(
            f_part_line[numpy.isfinite(f_part_line)])
        judge_line_list_lowprice = [gro_line,bt_line,pereturn_line]
    ...
    factor_df.to_csv('D:/基本面三击策略每日更新/nobad_stock_big.csv', encoding='gbk')
