...
def make_three_hit_factor_pool(stocks0, dates):
    ...
    #################################################
    # lowprice
    PBIndu_df = pd.read_csv('D:/基本面三击策略每日更新/uqer基本面因子/PBIndu.csv', index_col=0, encoding='gbk') \
        .reindex(index=dates, method='ffill').reindex(columns=stocks0).shift(1)
    PSIndu_df = pd.read_csv('D:/基本面三击策略每日更新/uqer基本面因子/PSIndu.csv', index_col=0, encoding='gbk') \
        .reindex(index=dates, method='ffill').reindex(columns=stocks0).shift(1)
    PEIndu_df = pd.read_csv('D:/基本面三击策略每日更新/uqer基本面因子/PEIndu.csv', index_col=0, encoding='gbk') \
        .reindex(index=dates, method='ffill').reindex(columns=stocks0).shift(1)
    DividendPS_df = pd.read_csv('D:/基本面三击策略每日更新/uqer基本面因子/DividendPS.csv', index_col=0, encoding='gbk') \
        .reindex(index=dates, method='ffill').reindex(columns=stocks0).shift(1)
    CapitalSurplusFundPS_df = pd.read_csv('D:/基本面三击策略每日更新/uqer基本面因子/CapitalSurplusFundPS.csv', index_col=0, encoding='gbk') \
        .reindex(index=dates, method='ffill').reindex(columns=stocks0).shift(1)
    ...
    for trade_date_num_i in range(len(trade_date_list)):
        ...
        ##########################################
        # lowprice
        f_part_line = -PBIndu_df.values[i]
        f_part_line = numpy.where(judge_line > 0, f_part_line, numpy.nan)
        PBIndu_line = arank(f_part_line, reverse=-1) / len(
            f_part_line[numpy.isfinite(f_part_line)])
        f_part_line = -PSIndu_df.values[i]
        f_part_line = numpy.where(judge_line > 0, f_part_line, numpy.nan)
        PSIndu_line = arank(f_part_line, reverse=-1) / len(
            f_part_line[numpy.isfinite(f_part_line)])
        f_part_line = -PEIndu_df.values[i]
        f_part_line = numpy.where(judge_line > 0, f_part_line, numpy.nan)
        PEIndu_line = arank(f_part_line, reverse=-1) / len(
            f_part_line[numpy.isfinite(f_part_line)])
        f_part_line = DividendPS_df.values[i]/close_real_df.values[i]
        f_part_line = numpy.where(judge_line > 0, f_part_line, numpy.nan)
        DividendPS_P_line = arank(f_part_line, reverse=-1) / len(
            f_part_line[numpy.isfinite(f_part_line)])
        f_part_line = CapitalSurplusFundPS_df.values[i]/close_real_df.values[i]
        f_part_line = numpy.where(judge_line > 0, f_part_line, numpy.nan)
        CapitalSurplusFundPS_P_line = arank(f_part_line, reverse=-1) / len(
            f_part_line[numpy.isfinite(f_part_line)])
        judge_line_list_lowprice = [PBIndu_line, PSIndu_line, PEIndu_line, DividendPS_P_line,
                                    CapitalSurplusFundPS_P_line]
    ...
    factor_df.to_csv('D:/基本面三击策略每日更新/nobad_stock_small.csv', encoding='gbk')
